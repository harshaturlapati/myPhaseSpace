...
int main(int argc, const char **argv)
{
	...
	string address = argc > 1 ? argv[1] : "192.168.1.230";
	OWL::Context owl;
	OWL::Markers markers;
	...
	while (owl.isOpen() && owl.property<int>("initialized"))
	{
		...
		else if (event->type_id() == OWL::Type::FRAME)
		{
			...
			log_file << "||" << "PStime:" << event->time() << " " << event->type_name() << " " << event->name() << "=" << event->size<OWL::Event>() << ":" << "||";

			if (event->find("markers", markers) > 0)
			{

				log_file << " markers=" << markers.size() << ":" << "||MARKER DATA BEGINS||";
				//cout << " markers=" << markers.size() << ":" << endl;
				for (OWL::Markers::iterator m = markers.begin(); m != markers.end(); m++)
					if (m->cond > 0)
						log_file << "mstart::" << "marker_" << m->id << "||X:" << m->x << "||Y:" << m->y << "||Z:" << m->z << "::m_end||";
				log_file << "MARKER DATA ENDS ||" << "t: " << GetTickUs() << endl;
